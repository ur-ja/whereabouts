-- Migrate anonymous entries to urjaarora5@gmail.com
-- Self-contained: applies required schema changes, then migrates data.
-- Sign up in the app with urjaarora5@gmail.com before running.

-- === Required schema (safe to re-run) ===
...
begin
  ...
  from auth.users
  where lower(email) = 'urjaarora5@gmail.com';

  if target_user_id is null then
    raise exception 'No user found for urjaarora5@gmail.com — sign up in the app first, then re-run this script';
  end if;
  ...
  insert into profiles (user_id, place_a, place_b)
  values (target_user_id, 'India', 'Australia')
  on conflict (user_id) do nothing;

  raise notice 'Migrated % entries to urjaarora5@gmail.com (user_id: %)', migrated_count, target_user_id;
end $$;
